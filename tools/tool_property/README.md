...
**调用方式**：在仓库根已 `pip install -e .` 时优先使用 `adk property …`，参数与在本目录执行 `python3 main.py …` 完全一致（`adk property --help` 等同 `python3 main.py --help`）。**一条龙**示例：`adk property BAIC`（对项目 `BAIC` 跑默认 fetch+generate+deploy）。仅输入 **`adk`** 进入交互菜单并选择 **property** 时，会先进入 **步骤 2 · 选择项目**（来自本目录 **`config.json`** 的 **`projects`**），再填写参数。平台 **`adk -h`** 含页眉（简介/版本/作者/版权）、**通用选项**、**工具包**、**交互选项**、**专业选项（一条龙）** 等分区（**`adk -v`** / **`adk --version`** 为平台版本号）。

CarPropertyManager 代码自动生成工具（v1.1.1） — 从飞书在线表格自动下载、生成 C/C++ 头文件，并可一键部署到目标 Git 仓库。
...
| 版本快照 | 每次下载前自动在飞书表格中创建命名版本快照，与本地文件一一对应，便于历史追溯 |
...
note bottom
  与 CLI 一致：可单独或组合执行 fetch / generate / deploy；
  指定动作后仅跑所选步骤（仍遵循各步内部校验与失败语义）
end note
...
**版本快照**：每次下载前，工具会自动在飞书表格的版本历史中创建一个同名快照（如 `car_property_list_bq_8775_20260406`），与本地文件一一对应。查看方式：打开飞书电子表格 → 右上角「`···`」→「查看已存版本」。
...
`fetch`、`generate`、`deploy` 是三个同级操作，可自由组合。每个操作都支持**长名**和**短 flag** 两种写法。不指定任何操作时，默认执行全部三步。

```bash
# 端到端一条龙（默认行为：fetch + generate + deploy）
python main.py                        # 全部项目
...
python main.py -fgd                   # 全部项目执行端到端一条龙，等同于python main.py
...
一条龙模式: python main.py  → 自动完成 fetch + generate + deploy
```
...
```text
获取表格原始标题 → 创建版本快照 → 导出下载 → 保存到 input/
```

1. **获取标题**：通过 Sheets API 读取表格的原始标题，用于文件命名
2. **版本快照**：调用 Drive 版本 API 创建命名版本（失败不中断，仅警告）
3. **导出下载**：优先使用 Drive Export API 异步导出 xlsx；若权限不足，自动降级为 Sheets API 逐 sheet 读取 + openpyxl 重建
4. **保存文件**：以 `{原始标题}_{YYYYMMDD}.xlsx` 格式保存到 `input/<项目名>/`
...
fetch 命令对飞书在线表格的操作**严格只读**：

- 版本快照：仅创建归档快照，不修改表格内容
- 导出下载：仅读取并导出，不修改表格内容
- 整个 fetch 流程中**没有任何写入或修改表格内容的 API 调用**
...
- 端到端一条龙（无参数默认执行 fetch + generate + deploy）
